...
(function () {
  ...
  function initWishlistToggles() {
    ...
    function updateButtons(productId, isActive) {
      document
        .querySelectorAll(
          '.tm-wishlist-toggle[data-product-id="' + productId + '"]',
        )
    ...
    document.addEventListener("DOMContentLoaded", function () {
      document.querySelectorAll(".cart .actions").forEach(function (td) {
        if (td.querySelector(".tm-cart-actions__inner")) return; // don't double-wrap

        var wrapper = document.createElement("div");
        wrapper.className = "tm-cart-actions__inner";

        var coupon = td.querySelector(".coupon");
        var updateBtn = td.querySelector('button[name="update_cart"]');

        if (coupon) wrapper.appendChild(coupon);
        if (updateBtn) wrapper.appendChild(updateBtn);
